...
//    }
    ...
    public String modifyUserInformationView(Model model) {
       ...
       /*11.07添加 */
        //将用户的数据库中的小区传到前端/
        String userCommunityGuid="";
        String userCommunity="";
        if(users1.getUserCommGuid()!=null){
            userCommunityGuid = users1.getUserCommGuid();
            userCommunity = communityMapper.selectByPrimaryKey(userCommunityGuid).getCommTitle();
        }
        ...
        //查询用户的家庭地址（下拉选），传到前端
        String userAddress = "";
        String province = users1.getUserProvince();
        String city = users1.getUserCity();
        String district = users1.getUserDistrict();
        userAddress = province + "," + city + "," + district;
//        System.out.println("用户的家庭地址是什么？"+userAddress);
        ...
        if (users1.getUserRole().equals("Tourist")) {
            model.addAttribute("userCommunity", userCommunity);
            return "updateUserInformation";
        } else {
            model.addAttribute("userCommunity", userCommunity);
            return "updateInformation";
...
//    }
    ...
    public String updateREQESTSave(@ModelAttribute @Valid Users users, Model model, @RequestParam(value = "img_z") MultipartFile file) throws IOException {
        ...
        /*11.07修改*/
        System.out.println("文件是否为空:" + file.getSize() + "或者" + file.isEmpty());
//        System.out.println("打印一下小区："+users.getUserProvince()+users.getUserCity()+users.getUserDistrict()+users.getUserCommGuid());
        CommunityExample communityExample = new CommunityExample();
